...
import HasKAL.StatisticsUtils.Correlation.DistCor(distcorU)
...
takeCorrelationCore method x y nshift = case method of
  ...
  DistCor ->  twoChannelData2DistCorV x y nshift
...
twoChannelData2DistCorV :: U.Vector Double -- ^ data Vector x
                        -> U.Vector Double -- ^ data Vector y
                        -> Int -- ^ number of shift to take correlation
                        -> U.Vector Double -- ^ Vector of correlation coefficient
twoChannelData2DistCorV x y nshift
  | U.length x == 0 = U.fromList []
  | U.length y == 0 = U.fromList []
  | nshift < 0      = U.fromList []
  | otherwise       = U.map (timeshiftedData2CorrelationV x y) $ U.fromList [-nshift..nshift]
  where timeshiftedData2CorrelationV :: U.Vector Double -> U.Vector Double -> Int -> Double
        timeshiftedData2CorrelationV x y n
          | n > dataLength = distcorU (dataHeadDropNV x dataLength) y
          | n < 0          = distcorU (dataHeadDropNV y (-n) ) x
          | otherwise      = distcorU (dataHeadDropNV x n    ) y
          where dataLength = max (U.length x) (U.length y)
...
correlationChunkCore :: CorrelationMethod -- ^ Pearson / MIC
                     ...
                     -> Int    -- ^ number of shift to take correlation
                     -> (S.Vector Double, S.Vector Double, S.Vector Double) -- ^ vector of correlation coefficient maximized ([time], [rho_max], [timeshift])
correlationChunkCore method x y ttotal tchunk fs nshift
  | U.length x == 0 = emptyResult
  | U.length y == 0 = emptyResult
  | fs < 0          = emptyResult
  | nshift < 0      = emptyResult
  | otherwise       = execute method x y nchunk fs nshift
   where nx     = U.length x :: Int
         ...
         execute :: CorrelationMethod -> U.Vector Double -> U.Vector Double -> Int -> Double -> Int -> (S.Vector Double, S.Vector Double, S.Vector Double)
         execute method x y nchunk fs nshift = do
           let result   = transpose $ map (correlationChunkCore method x y nchunk fs nshift ) nchunk_list :: [[Double]]
         ...
         correlationChunkCore :: CorrelationMethod -> U.Vector Double -> U.Vector Double -> Int -> Double -> Int -> Int-> [Double]
         correlationChunkCore method x y nchunk fs nshift index = do
           let rhov | method==Pearson = twoChannelData2CorrelationV
                        (U.slice (nchunk*index) nchunk x) (U.slice (nchunk*index) nchunk y) nshift :: U.Vector Double
                    | method==MIC = twoChannelData2MICV
                        (U.slice (nchunk*index) nchunk x) (U.slice (nchunk*index) nchunk y) nshift :: U.Vector Double
                    | method==DistCor = twoChannelData2DistCorV
                        (U.slice (nchunk*index) nchunk x) (U.slice (nchunk*index) nchunk y) nshift :: U.Vector Double

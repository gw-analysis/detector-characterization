{-******************************************************************
  ...
  * Last Modified: 2014/01/24 11:43:58
  ******************************************************************-}

module HasKAL.GUI_Utils
  (hasKalGuiTop
   --,hasKalGuiGlitch
  ) where
...
import HasKAL.Monitor.EXTKleineWelle
import HasKAL.PlotUtils
...
hasKalGuiGlitch activeSubSystemlabels = do
  ...
  onClicked glitchExecute $ do
    ...
    lwtOutput <- extKleineWelle kwStride kwBasename kwTransientDuration kwSignificance kwThreshold kwDecimateFactor glitchActiveLabels kwUnowen_1 kwFsample kwUnowen_2 kwOptFilePref kwListFile kwGpsTime kwActiveLabels
    let lwtColmunNum = length kwActiveLabels
    putStrLn "Run Plot tool"
    if lwtColmunNum == 2
      then forM [0..(length glitchActiveLabels)-1] $ \lambda -> do
             scatter_plot_2d "TITLE 2" "HOGEHOGE" 10.0 (640,480) (convert_StoDT2L (lwtOutput !! lambda))
      else if lwtColmunNum == 3
        then forM [0..(length glitchActiveLabels)-1] $ \lambda -> do
               scatter_plot_3d "TITLE 3" "HOGEHOGE" 10.0 (640,480) (convert_StoDT3L (lwtOutput !! lambda))

...
chebyshev1 n r fs fc filterType = do
  ...
  (map realPart numeratorList, map realPart denominatorList )
  where
...
h1 n r
  | odd n  = foldl (\acc k -> acc * (-1)*filterPole n k r) 1.0 [1..n]
  | even n = realToFrac (10.0**(r/20.0)) * foldl (\acc k -> acc * (-1)*filterPole n k r) 1.0 [1..n]
...
h0Core r fs fc n filt
  ...
  | filt==High = foldl (\acc m' -> acc * 2.0/(realToFrac (2.0*pi*fc/fs) - 2.0*filterPole n m' r)) 1.0 [1..n]


filterPole :: Int -> Int -> Double -> Complex Double
filterPole n' k' r = realToFrac ((((1/gam)-gam)/2.0) * sin (pi*(2*k-1)/(2*n)))
  + jj * realToFrac ((((1/gam)+gam)/2.0) * cos (pi*(2*k-1)/(2*n)))
  where n = fromIntegral n' :: Double
        k = fromIntegral k' :: Double
        gam = ((1.0+sqrt (1.0+epcilon**2.0))/epcilon)**(1.0/n)
        epcilon = sqrt (10.0**(r/10.0)-1.0)

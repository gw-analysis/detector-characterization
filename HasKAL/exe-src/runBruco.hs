...
main = do
  ...
  let gps = read $ time2gps $ year+-+month+-+day++" "++hour+:+minute+:+second++" JST"
      duration = read dur :: Int
      -- for Bruco
      fftLength = read fftsec :: Double -- seconds
      -- for Result
      oFile = (drop 3 ch1)++"_"++year+-+month+-+day+-+hour+-+minute+-+second++"_"++dur++"_Bruco.html"

  {-- read data --}
  mbWd1 <- kagraWaveDataGetC (fromIntegral gps) (fromIntegral duration) ch1
  let wss1 = case mbWd1 of
             Just x  -> x
             Nothing -> error $ "Can't find data: "++ch1++"-"++year++"/"++month++"/"++day++"-"++hour++":"++minute++"'"++second++"''"

...
main = do
 ...
 (chunklen', fs', t0') <- case length args of
  ...
  _ -> error "Usage: runRangeMonIMBHSTDIN chunklen fs t0 STDIN"
 ...
 let hf = gwspectrogramWaveData 0 fftLength wd
     ...
     ir'' = map ((0.44*) . distImbh 100 100) x
     ir'  = map infinityTo0 ir'' :: [Double]
     ir   = V.fromList ir'
     vecT_hr = V.map (1*) vecT
-- print $ V.toList ir
-- print $ V.toList vecT_hr
 plotXV Linear Line 1 RED (xlabel, "IMBH Range[Mpc]") 0.05 title ((0,0),(0,0)) $ (vecT_hr, ir)
-- let xir = V.toList ir
--     xt  = V.toList vecT_hr

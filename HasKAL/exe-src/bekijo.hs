  ...
  (varOpt, varArgs) <- getArgs >>= \optargs ->
    case getOpt Permute options optargs of
      ...
      (_  , _, errs) -> ioError (userError (concat errs ++ usageInfo header options))
        where header = "Usage: squared [-t(--withTime)] val stdin"

  {-- parameters --}
  val' <- case (length varArgs) of
    1 -> return (head varArgs)
    _ -> error "Usage: squared [-t(--withTime)] val stdin"
  ...
  case optTime varOpt of
    ...
    True  -> do
      ...
      mapM_ (\(x,y) -> hPutStrLn stdout $ (show x)++" "++show y)
               $ zip (V.toList (head inputPart)) (V.toList (V.map (**val) (inputPart!!1)))
...
options =
  [ Option ['t'] ["withTime"]
      ...
      "Usage: squared [-t(--withTime)] val stdin"
  ]

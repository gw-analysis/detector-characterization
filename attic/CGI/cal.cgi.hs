...
cgiMain = do
  ...
  output $ htmlcal (yyyy,mm,dd)

htmlcal :: (Int, Int, Int) -> String
htmlcal (yy, mm, dd) = concat [
  "<html>",
    "<head>",
    ...
    "<body>",
      "<h1>Calendar</h1>",
      concat $ map (htmlcalCore (yy, mm, dd)) $ reverse dateRange,
    "</body>",
  "</html>"
  ]
...
htmlcalCore :: (Int, Int, Int) -> (Int, Int) -> String
htmlcalCore today (yy, mm) = concat [
  "<table>",
    ...
    "<tr><th>",
      "<table style=\"font-size:80%;\">",
        "<tr align=\"right\"><td>Mon.</td><td>Tue.</td><td>Wed.</td><td>Thu.</td><td>Fri.</td><td>Sat.</td><td>Sun.</td></tr>",
          monthlyLink today (yy, mm),
      "</table>",
    "</th></tr>",
  "</table><br>"
  ]

monthlyLink :: (Int, Int, Int) -> (Int, Int) -> String
monthlyLink today (yyyy, mm) = concat $ newline $ map (dailyLink today) dateList
...
dailyLink :: (Int, Int, Int) -> (Int, Int, Int) -> String
dailyLink today (yyyy, mm, dd) 
  | dd == 0                 = "<td></td>"
  | (yyyy, mm, dd) >= today = "<td align=\"right\">"++(show dd)++"&ensp;</td>"
  | otherwise               = "<td align=\"right\"><a href=\""++linkfile++"\" target=\"right_frame\">"++(show dd)++"</a>&ensp;</td>"
  where linkfile = (show yyyy)++"-"++(show0 mm)++"-"++(show0 dd)++".html"

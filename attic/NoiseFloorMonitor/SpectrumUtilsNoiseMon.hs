...
gwpsdOneshot :: [Double]-> Int -> Double -> [(Double, Double)]
gwpsdOneshot dat nfft fs = gwpsdCoreOneshot MedianAverage dat nfft fs Hann

gwpsdCoreOneshot :: PSDMETHOD -> [Double] -> Int -> Double -> WindowType -> [(Double, Double)]
gwpsdCoreOneshot method dat nfft fs w
  | method==Welch = gwpsdWelch dat nfft fs w
  | method==MedianAverage = gwpsdMedianAverageCoreOneshot dat nfft fs w
  | otherwise =  error "No such method implemented. Check GwPsdMethod.hs"

gwpsdMedianAverageCoreOneshot :: [Double] -> Int -> Double -> WindowType -> [(Double, Double)]
gwpsdMedianAverageCoreOneshot dat nfft fs w = do
  let dat' = fromList dat
      (fv, psdv) = gwpsdMedianAverageCoreVOneshot dat' nfft fs w
  zip (toList fv) (toList psdv)

gwpsdMedianAverageCoreVOneshot :: Vector Double -> Int -> Double -> WindowType -> (Vector Double, Vector Double)
gwpsdMedianAverageCoreVOneshot dat nfft fs w = do
  let --ns = 2*(length dat) `div` nfft - 1 :: Int
      ...
      poweroneshot = map (*scale_psd).toList $ (psdOdd dat nfft w) !! 100 
  (fvec, fromList poweroneshot)



gwpsd :: [Double]-> Int -> Double -> [(Double, Double)]
gwpsd dat nfft fs = gwpsdCore MedianAverage dat nfft fs Hann
